...
Each task folder is the single home for all artifacts belonging to that plan: the plan file, specs, iteration reports, feasibility verdicts, and results.tsv. Do NOT create `reports/` or `references/` sibling dirs — all colocated artifacts go inside the task folder.
...
- Reports or references (colocate inside the task folder, not as a sibling dir)
